...
async def set_timer(message: types.Message):
    ...
    if line.isdigit():
        ...
        cur.execute("INSERT INTO users_calls VALUES (?, ?)", [user_id, vrem])
        base.commit()
        await message.answer('таймер на ' + line + ' минут установлен' + str(user_id))
    else:
        await message.answer('Syntax.Error')



async def spamer(timer):
    while True:
        ...
        for row in kp:
            ...
            if tstamp < int(time.time()):
                ...
                cur.execute("DELETE FROM users_calls WHERE (iser_id = ?) AND (timestamp = ?)", [user_id, tstamp])
                base.commit()
            else:
                break
